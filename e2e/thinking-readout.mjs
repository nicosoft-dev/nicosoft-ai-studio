...
// Backfill any missing key + bind editor (plain chat — fastest path to a streaming state).
await page.evaluate(
  async ({ key, model }) => {
    ...
    await window.api.roles.setBinding('editor', { endpointId: gemini.id, model })
  },
...
console.log('✓ thinking readout: dot, breathing not spinning, avatar static, 10px gap above')
