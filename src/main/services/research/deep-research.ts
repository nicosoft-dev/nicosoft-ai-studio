// Studio research — the bundled DEEP-RESEARCH orchestration script: a byte-faithful port of Claude Code's
// `deep-research` Workflow harness (2.1.206). Same 5 phases (Scope → Search → Fetch → Verify → Synthesize),
// same constants (3 votes / 2 refutations-to-kill / 15 fetches / 25 verify-claims), and the same THREE-STATE
// verify (survives / refuted / unverified — an infra failure must NOT read as "refuted"). It runs on the shared
// node:vm script executor (services/script) exactly like the lens code-review template; the ONLY studio
// substitution is the sub-agent kit — WebSearch/WebFetch instead of git/grep — injected by the research
// spawnAgent consumer (agent-research.ts). The research question arrives as `args` (the /research command
// supplies it; empty args returns an error, matching CC).
//
// ALIGNMENT BASELINE: the body below is a VERBATIM copy of the CC script
// (docs/script-orchestration-alignment.md appendix A) with only a leading `export const meta` prepended (the
// studio executor requires meta as the first statement). Keep it in sync with CC; do not diverge the algorithm.
