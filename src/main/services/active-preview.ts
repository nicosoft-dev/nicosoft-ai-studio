...
// Security: loadURL on the live guest is the ONE navigation path that escapes the initial-src whitelist
// (isPreviewSrcAllowed in index.ts only guards the webview's FIRST src). Both entry points — the renderer URL
// bar (openPreview) and the agent's preview_navigate (settlePending) — funnel through here, the single chokepoint
// for every live navigation. Enforce http/https only (blocks file:// local-file reads + data:/app:/javascript:)
// and reject link-local 169.254/16 + fe80::/10 — i.e. cloud instance metadata (169.254.169.254), which has no
// legitimate preview use and would leak cloud credentials. localhost / private LAN stay allowed on purpose:
// previewing a local dev server (http://localhost:5173) is a core feature. (DNS-rebinding to an internal IP would
// need network-layer filtering — out of scope for this URL chokepoint.)
function assertPreviewUrlAllowed(url: string): void {
  let parsed: URL
  try {
    parsed = new URL(url)
  } catch {
