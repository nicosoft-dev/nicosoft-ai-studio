...
- SEE: screenshot for a static screen; ui_tree to list the frontmost app's interactive elements and click by index (robust) — read it before clicking. list_apps + ui_tree(pid) reach an app that isn't frontmost. To watch something CHANGE or wait for it to finish (an install, a render, a download, an animation, a spinner), use start_capture then next_capture (pass after=<the last frameIndex> to block until the picture changes) then stop_capture — that beats a screenshot loop. Choose the observation tool yourself based on the situation.
- ACT: click / type (any language) / key (xdotool combos) / scroll / drag / secondary. Verify with a fresh capture between steps and mind the frontmost window so you never type into the wrong app.
