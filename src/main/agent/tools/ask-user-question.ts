...
//
// Schema tolerance (audit F7): models trained on Claude Code send the `questions[]` shape with OBJECT
// options ({label, description, preview}); the original Studio shape was a single flat question with bare
// string options, so a CC-trained call was hard-rejected. We accept BOTH and normalize to Studio's
...
// An option is a bare label (legacy) OR Claude Code's object form {label, description?, preview?}.
...
// Accept Claude Code's `questions[]` shape AND a single flat question (legacy), so neither is rejected.
const inputSchema = z.object({
  questions: z.array(questionSchema).min(1).max(4).optional().describe('One or more questions to ask (Claude Code shape)'),
...
export const askUserQuestionTool = buildTool<typeof inputSchema, string>({
  ...
  async call(input, ctx) {
    ...
    // Normalize the CC `questions[]` shape and the legacy single-question shape into one list.
