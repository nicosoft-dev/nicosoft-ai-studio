...
export const todoTool = buildTool<typeof inputSchema, TodoOutput>({
  ...
  prompt: () =>
    ...
    'leave it in_progress and add a new item for what is blocking. When the WHOLE task is finished, do a FINAL ' +
    'TodoWrite that marks every done item completed — never end leaving a finished task stuck in_progress (after a ' +
    'resumed async run, when you incorporate the result and wrap up, this close-out is the easy step to forget). ' +
