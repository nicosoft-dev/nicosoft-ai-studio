  ...
  return {
    ...
    compactNow: async (convId) => {
      ...
      set((s) => {
        ...
        for (let j = msgs.length - 1; j >= 0; j--) if (msgs[j].role === 'assistant' && msgs[j].segmentKind !== 'workflow-launch' && msgs[j].segmentKind !== 'workflow-draft' && msgs[j].segmentKind !== 'research-launch') { i = j; break }
