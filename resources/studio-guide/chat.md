...
- **Send while it's working**: you don't have to wait or press Stop — type and hit Enter while the reply is still streaming. The message joins the conversation immediately and the running agent reads it at its next step, adjusting course without restarting anything. If the run happens to finish just as you send, the message simply starts a new turn. In a coordinator conversation while a **collaboration** is running, a plain message goes to the whole team and `@Name …` goes to that one expert (a working expert picks it up at its next step; a waiting one wakes up for it). While Danny is routing or running a dispatch, there is no mid-step to interrupt — the message still posts right away and is answered as soon as that turn finishes. Text only while streaming (attach images between turns).
...
- **Context window panel**: click the ring to see what the prompt is made of — System prompt, Auto-memory, System tools, Messages and Free space, as a stacked bar plus a legend with tokens and percentages. The total is the size the API itself charged for your last turn, so it is the real one; it is still labelled "Estimated usage by category" because splitting that total into categories means measuring each part on its own and letting the tool kit fall out as whatever is left over, which carries a little error even when the total does not. The split is measured per turn, persists across restarts, and — for a conversation that has history but no stored split yet — is computed on demand the moment you open it (free token-counting probes, same as Claude Code's `/context`). Only a brand-new conversation (nothing to measure) or one right after `/compact` shows the total with a "details appear after the next turn" note.
