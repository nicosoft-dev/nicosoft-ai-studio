...
- **Context window panel**: click the ring to see what the prompt is made of — System prompt, Auto-memory, System tools, Messages and Free space, as a stacked bar plus a legend with tokens and percentages. The total is the size the API itself charged for your last turn, so it is the real one; it is still labelled "Estimated usage by category" because splitting that total into categories means measuring each part on its own and letting the tool kit fall out as whatever is left over, which carries a little error even when the total does not. The category split needs a measured turn: before one has run in this app session (or right after `/compact`), the panel opens with the total and a note saying the details appear after the next turn.
