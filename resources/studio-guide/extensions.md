...
- **Computer use** (`ns_computer_use`, macOS only) — lets any expert see and control your Mac: screenshot the screen (or stream it live to watch something change), read on-screen elements, click, type (any language), scroll, and drag across native apps, not just the browser. A global switch turns it on for every expert. It runs through a small helper app ("NicoSoft Computer Use") that needs two macOS permissions — Accessibility and Screen Recording — and the card shows whether the helper is installed/running and each permission's state, with an "Open settings" shortcut to grant them. While an expert is controlling the Mac a banner shows on screen; press Esc to stop at any time.
