...
You can add custom roles ("New Role" in the sidebar) with their own system prompt, model and greeting. A custom role is a chat persona by default. Switch on **Agent capability** in its editor — or directly on its profile page, where the same switch and tool groups edit in place and save instantly — to make it a real agent under its own persona: it gains a working-folder picker, an "agent" badge, and a tool kit you pick by capability group — Read files / Write & edit files / Web access / Run code / Scheduled tasks are on by default; Shell commands (runs real commands in the working folder — enable deliberately), Generate images, Export PDF and Sub-agents are opt-in. Write & edit implies Read. An agent-enabled custom role is a full team member: Danny can route work to it by name or fit (its persona's first line + tool groups appear in his roster), you can `@name` it directly, it can join collaborations and workflow steps, and it can be a scheduled-task step executor. It needs a model that supports tool use.
